...
public final class SortInventory {
        ...
        for (int slot : inventorySlots) {
            ...
            if (!stack.isEmpty() && stack.getCount() < stack.getItem().getDefaultMaxStackSize()) {
                ...
                for (int tempSlot = slot + 1;
                        Interaction.getCursorStack().getCount()
                                        < Interaction.getCursorStack().getItem().getDefaultMaxStackSize()
                                && tempSlot <= inventorySlots.getLastSlot()
                                && !Interaction.getCursorStack().isEmpty();
                        tempSlot++) {
                    if (ItemStack.isSameItemSameComponents(
                            Interaction.getCursorStack(),
                            screenHandler.getSlot(tempSlot).getItem())) {
        ...
        return switch (InventoryTweak.CONFIG.sortType) {
            case NAME ->
                inventorySlots.stream()
                        .filter(slot -> !screenHandler.getSlot(slot).getItem().isEmpty())
                        .sorted(Comparator.comparing((Integer slot) -> screenHandler
                                        .getSlot(slot)
                                        .getItem()
                                        .getDisplayName()
                                        .getString())
            ...
            case TYPE ->
                inventorySlots.stream()
                        .filter(slot -> !screenHandler.getSlot(slot).getItem().isEmpty())
                        .sorted(Comparator.comparing((Integer slot) -> Item.getId(
                                        screenHandler.getSlot(slot).getItem().getItem()))
                                .thenComparing((Integer slot) -> screenHandler
                                        .getSlot(slot)
                                        .getItem()
                                        .getDisplayName()
                                        .getString())
            ...
            case RARITY ->
                inventorySlots.stream()
                        .filter(slot -> !screenHandler.getSlot(slot).getItem().isEmpty())
                        .sorted(Comparator.comparing((Integer slot) ->
                                        screenHandler.getSlot(slot).getItem().getRarity())
                                .reversed()
                                .thenComparing((Integer slot) -> screenHandler
                                        .getSlot(slot)
                                        .getItem()
        ...
        for (int slot : inventorySlots) {
            if ((!Interaction.hasEmptyCursor())
                    && (ItemStack.isSameItemSameComponents(
                                    screenHandler.getSlot(slot).getItem(), Interaction.getCursorStack())
                            || screenHandler.getSlot(slot).getItem().isEmpty())) {
                Interaction.clickStack(slot);
            }
        }
    }
